...
start_node() {
	...
	init_node "$datadir"
	echo "Starting node${idx}: http=${http_port}, p2p=${p2p_port}, nodepartition=${node_partition}"
...
for idx in 1 2 3 4; do
	...
	echo "node${idx}: http://127.0.0.1:${port} block=${block:-?} peers=${peers:-?} log=${RUN_DIR}/node${idx}.log"
done
